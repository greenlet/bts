...
parser.add_argument('--num_minibatches', type=int, help='number of minibatches', default=4)
parser.add_argument('--minibatch_size', type=int, help='minibatch size', default=2)
...
def list_trainable_vars():
    filter_out = []
    if args.fix_first_conv_blocks or args.fix_first_conv_block:
        filter_out = ['conv1', 'encoder/densenet161/BatchNorm', 'dense_block1', 'transition_block1']
        if args.fix_first_conv_blocks:
            filter_out += ['dense_block2', 'transition_block2']
    def filter_op(var):
    ...
    if len(filter_out):
        train_vars = filter(filter_op, train_vars)

    return list(train_vars)
...
def train(params):
    with tf.Graph().as_default(), tf.device('/gpu:0'):
        ...
        batch_size = args.num_minibatches * args.minibatch_size
        ...
        learning_rate = tf.train.polynomial_decay(start_learning_rate, global_step, num_total_steps, end_learning_rate,
                                                  0.9)
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            with tf.device('/gpu:0'):
                ...
                model = BtsModel(params, args.mode, image, depth_gt, focal=focal, bn_training=False)

                train_vars = list_trainable_vars()
                # for var in train_vars:
                #     print(var.name)

                total_loss = tf.Variable(0.0, trainable=False)
                zero_total_loss = total_loss.assign(0.0)
                accum_total_loss = total_loss.assign_add(model.total_loss)
                total_loss_op = tf.divide(accum_total_loss, args.num_minibatches)

                accum_vars = [tf.Variable(tf.zeros_like(var.initialized_value()), trainable=False) for var in train_vars]
                zero_accum_vars = [var.assign(tf.zeros_like(var)) for var in accum_vars]

                grads = opt_step.compute_gradients(model.total_loss, var_list=train_vars)

                accum_grads = [accum_vars[i].assign_add(grad[0]) for i, grad in enumerate(grads)]
                accum_grads_to_apply = [(accum_vars[i] / args.num_minibatches, grad[1]) for i, grad in enumerate(grads)]
                apply_gradients_op = opt_step.apply_gradients(accum_grads_to_apply, global_step=global_step)



        tf.summary.scalar('learning_rate', learning_rate, ['model_0'])
        tf.summary.scalar('total_loss', model.total_loss, ['model_0'])
        ...
        for step in range(start_step, num_total_steps):
            ...
            sess.run([zero_accum_vars, zero_total_loss])
            for step_mini in range(args.num_minibatches):
                # print('minibatch', step_mini)
                sess.run([accum_grads, accum_total_loss])

            _, lr, loss_value = sess.run([apply_gradients_op, learning_rate, total_loss_op])

            print('step: {}/{}, lr: {:.12f}, loss: {:.12f}'.format(step, num_total_steps, lr, loss_value))

            duration += time.time() - before_op_time
            if step and step % 100 == 0:
                examples_per_sec = batch_size / duration * 100
                duration = 0
